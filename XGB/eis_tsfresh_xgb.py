...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", 
        help="Only 'train_clf', 'train_reg' or 'pred' are valid inputs. You need to train the model beofre calling prediction\
            if you pass no argument, the cleassifier and the predictor will be trained and predicitons will be made")
    parser.add_argument(
        "--modelpath", help="Path where to store/load the model from")
    parser.add_argument(
        "--datapath", help="Path where to load the data from. In this directory, there must be a file\
            called train_data.csv adn test_data.csv")
    parser.add_argument(
        "--pred_data", help=".csv file to use for making predictions. \
            If no argument is supplied, the script will look for test_data.csv under datapath")
