...
def create_cnn_model(num_classes: int, initial_lr: float = 0.01, features: list = []):
    ...
    image_inputs = layers.Input(shape=(128, 128, 3), name="image")
...
with mlflow.start_run():
    ...
    h = model.fit(train_ds, validation_data=val_ds, epochs=150, callbacks=callbacks)
...
eis_data.to_csv("128_results.csv")
