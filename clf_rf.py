...
def baseline_model(train_data_f, test_data_file, output_dir, compare_models=True, cross_val=False, save=False):
    ...
    if compare_models:
        ...
        simple_model = RandomForestClassifier(class_weight='balanced_subsample', n_estimators=800, n_jobs=-1, random_state=42)

        params_dict = {'bootstrap': True, 'max_depth': 75, 'max_features': None, 'min_samples_leaf': 1, 'min_samples_split': 3, 'n_estimators': 600}
        gs_model = RandomForestClassifier(class_weight='balanced_subsample', n_jobs=-1, random_state=42, **params_dict)

        # Compare the performance of the model with only the number of estimators tuned and the larger grid search
        gs_model_score = cross_val_score(gs_model, X_train, y_train, cv=5, n_jobs=-1, scoring='f1_macro')
        default_model_score = cross_val_score(simple_model, X_train, y_train, cv=5, n_jobs=-1, scoring='f1_macro')
...
if __name__ == "__main__":
    ...
    baseline_model(train_data_f, test_data_f, output_dir, compare_models=False, cross_val=None, save=True)    
    print('Done')
